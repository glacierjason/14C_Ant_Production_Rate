...
for a = 1:length(Rc)
    for b = 1:length(x)
        ...
        c4 = a5 + a6.*x(b)./(1 + a7.*exp(a8.*x(b))); % lethargy^-1
        c12 = a9.*(exp(-a10.*x(b)) + a11.*exp(-a12.*x(b))); % MeV

        PhiLmin = a1min.*(exp(-a2min.*x(b)) - a3min.*exp(-a4min.*x(b))); %length of Rc
        PhiLmax = a1max.*(exp(-a2max.*x(b)) - a3max.*exp(-a4max.*x(b))); %length of Rc
