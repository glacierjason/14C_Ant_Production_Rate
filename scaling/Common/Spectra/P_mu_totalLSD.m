function out = P_mu_totalLSD(z,h,Rc,SPhi,consts,dflag)
...
% 
% syntax out = P_mu_totalSato(z,h,Rc,SPhi,consts,dflag);
%
...
% for the function "P_mu_total.m" as part of Balco et al., 2008, 
%
...
% %solar modulation potential (SPhi) to Sato Force Field Potential due to different
...
for a = 1:length(z)
    ...
    temp = integral(@(x) Rv0(x).*ppval(RzSpline,x),z(a),(2e5+1),'RelTol',tol); 
...
% Per John Stone, personal communication 2011
aalpha = 1.0;
Beta = 1.0;
% 
...
sigma0_LS = consts.sigma0.LS;
sigma0_Lm = consts.sigma0.Lm;
sigma0_St = consts.sigma0.St; 
fstar.LS = consts.fstar.LS;
fstar.Lm = consts.fstar.Lm;
fstar.St = consts.fstar.St;
Natoms = consts.Natoms;
k_negpartial = consts.k_negpartial;

% fast muon production

P_fast_LS = phi.*Beta.*(Ebar.^aalpha).*sigma0_LS.*Natoms;
P_fast_Lm = phi.*Beta.*(Ebar.^aalpha).*sigma0_Lm.*Natoms;
P_fast_St = phi.*Beta.*(Ebar.^aalpha).*sigma0_St.*Natoms;

% negative muon capture

P_neg_LS = R.*k_negpartial.*fstar.LS;
P_neg_Lm = R.*k_negpartial.*fstar.Lm;
P_neg_St = R.*k_negpartial.*fstar.St;

% return 

if strcmp(dflag,'no')
    out.LS = P_fast_LS + P_neg_LS;
    out.Lm = P_fast_Lm + P_neg_Lm;
    out.St = P_fast_St + P_neg_St;    
...
elseif strcmp(dflag,'yes')
    ...
    out.Beta = Beta;
    out.Ebar = Ebar;
    out.P_fast_LS = P_fast_LS;
    out.P_neg_LS = P_neg_LS;
    out.P_fast_Lm = P_fast_Lm;
    out.P_neg_Lm = P_neg_Lm;
...
% http://pdg.lbl.gov/2010/AtomicNuclearProperties/MUON_ELOSS_TABLES/muonloss_281.pdf
