function N = NeutronsCD(h,Rc,s,w,consts,ND) 
...
%
%Modified by Allie Koester and Nat Lifton, 2022
%Purdue University, koestea@purdue.edu
%requires input from numdCD function which calculates number density for
%elements of samples
...
%Rc = 0; %added to get to SLHL ref prod
...
for a = 1:length(Rc)
    ...
    %starts at 1 MeV 
    NSconst = 1e-27.*3.1536e7; %nuclide-specific production rate integration constants
     
    %Calculate theoretical element-specific production for sample
    %Si 
        N.P14Si(a) = (trapz(E(clipindex:end),PhiGMev(clipindex:end).*consts.Si28nxC14(clipindex:end)).*ND(1,1)).*NSconst;
    %Ti
        N.P14Ti(a) = (trapz(E(clipindex:end),PhiGMev(clipindex:end).*consts.Ti48nxC14(clipindex:end)).*ND(1,2)).*NSconst;
    %Al
        N.P14Al(a) = (trapz(E(clipindex:end),PhiGMev(clipindex:end).*consts.Al27nxC14(clipindex:end)).*ND(1,3)).*NSconst;
    %Fe1 = Fe2O3
        N.P14Fe1(a) = (trapz(E(clipindex:end),PhiGMev(clipindex:end).*consts.Fe56nxC14(clipindex:end)).*ND(1,4)).*NSconst; 
    %Fe2 = FeO
        N.P14Fe2(a) = (trapz(E(clipindex:end),PhiGMev(clipindex:end).*consts.Fe56nxC14(clipindex:end)).*ND(1,5)).*NSconst;
    %Mn
        N.P14Mn(a) = (trapz(E(clipindex:end),PhiGMev(clipindex:end).*consts.Mn55nxC14(clipindex:end)).*ND(1,6)).*NSconst;
    %Mg
        N.P14Mg(a) = (trapz(E(clipindex:end),PhiGMev(clipindex:end).*consts.Mg24nxC14(clipindex:end)).*ND(1,7)).*NSconst;
    %Ca
        N.P14Ca(a) = (trapz(E(clipindex:end),PhiGMev(clipindex:end).*consts.Ca40nxC14(clipindex:end)).*ND(1,8)).*NSconst;
    %Na
        N.P14Na(a) = (trapz(E(clipindex:end),PhiGMev(clipindex:end).*consts.Na23nxC14(clipindex:end)).*ND(1,9)).*NSconst;
        %N.P14Na(a) = (trapz(E(clipindex:end),PhiGMev(clipindex:end).*consts.Na23nxC14TJ(clipindex:end)).*ND(1,9)).*NSconst; 
        %spliced TENDL+JENDL

    %K
        N.P14K(a) = (trapz(E(clipindex:end),PhiGMev(clipindex:end).*consts.K39nxC14(clipindex:end)).*ND(1,10)).*NSconst;
    %P
        N.P14P(a) = (trapz(E(clipindex:end),PhiGMev(clipindex:end).*consts.P31nxC14(clipindex:end)).*ND(1,11)).*NSconst;
        %N.P14P(a) =(trapz(E(clipindex:end),PhiGMev(clipindex:end).*consts.P31nxC14T(clipindex:end)).*ND(1,11)).*NSconst; %TENDL

    %O
        N.P14O(a) = (trapz(E(clipindex:end),PhiGMev(clipindex:end).*consts.O16nn2pC14(clipindex:end)).*ND(1,12)).*NSconst;

    %Total theoretical P14 from neutron spallation
    N.P14CD(a) = N.P14Si(a) + N.P14Ti(a) + N.P14Al(a) + N.P14Fe1(a) + N.P14Fe2(a) + N.P14Mn(a) + N.P14Mg(a) + N.P14Ca(a) +...
        N.P14Na(a) + N.P14K(a) + N.P14P(a) + N.P14O(a);
